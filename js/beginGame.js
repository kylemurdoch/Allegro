...
function playNote(e) {
  ...
  if (keyNote === curNote) {
    ...
    //get HTML tag of curNote key
    for (var i = 0; i < keys.length; i++) {
      if(keys[i].getAttribute("data-note") === curNote){
        console.log(keys[i].getAttribute("data-note"));
        //keys[i].innerHTML = 5;
        /*
        $(keys[i]).bind('animationend webkitAnimationEnd MSAnimationEnd oAnimationEnd', function () {
          $(keys[i]).removeClass('wrong');
        })
        $(keys[i]).addClass('wrong');*/
        $(keys[i]).fadeOut(150).fadeIn(150); 
